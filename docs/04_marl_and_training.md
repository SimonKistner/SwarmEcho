...
- **Actor communication** (`network.memory_comm_enabled: true`) replaces the plain GRU input with a single-round TarMAC update:
  ...
  ```
  With `network.memory_comm_frequency_control: static`, every reachable sender may broadcast during each `memory_comm_every_k_steps` communication slot. With `network.memory_comm_frequency_control: ic3`, the same communication slots are used, but a learned Bernoulli sender gate decides which agents broadcast their TarMAC token in that slot; PPO optimizes the gate log-probability together with the physical action log-probability. `tarmac_include_self` adds the receiver's own previous token only when the receiver has at least one external agent message to attend over, and it is independent of that receiver's own sender gate. The base station is passive: it stores the first target-knowing reporter's emitted TarMAC signature/value pair, then replays that saved token to non-knowing agents that are in base communication range. Base replay remains governed by `memory_comm_every_k_steps`, and no previous action is fed back into the actor GRU or communication state.
