...
These metrics are emitted only when `network.memory_comm_frequency_control: ic3` is active. IC3 gates agent-agent TarMAC sender tokens; physical/wall-aware range still comes from the environment adjacency matrix, and base replay still follows `memory_comm_every_k_steps`.

- `comm/ic3_rate_mean`: Mean completed-episode communication rate in the training sliding window. For each completed episode, this is `sum_t sum_i gate_i_t / (episode_length * num_agents)`, with inactive agents counted as not communicating.
...
- `comm/ic3_rate_by_step/step_XXX`: Rollout-step gate rate for step `XXX` within the latest PPO rollout, averaged over vectorized environments and agents at that rollout index. This is not episode-normalized and is intended as a coarse temporal shape diagnostic.
- `comm/ic3_rate_when_target_known`: Mean completed-episode gate rate restricted to steps where at least one agent already knew the target. Episodes with no such step use a safe zero denominator contribution.
...
- `eval/comm/ic3_rate_mean`: Deterministic-evaluation communication rate using the same episode-length normalization as training.
