...
def scan(args):
    ...
    graph = shared_roadmap(building.solid_min_m, building.solid_max_m, lower, upper,
                          clearance, plan=True, merge_walls=cfg.roadmap_merge_walls,
                          edge_spacing=building.cell_size_m, corner_bonus_m=cfg.roadmap_corner_bonus_m,
                          corner_merge_distance_m=building.wall_thickness_m + 2 * clearance)
    ...
    target_lo, target_hi, _ = _target_spawn_boxes(building, cfg)
    ...
    def report(label, eligible, indices, values, longest=False, save=True):
        ...
        if save:
            path = generate(output / f"{stem}_{label}.roadmap.json", level_name=args.level,
                            ...
                            corner_bonus_m=cfg.roadmap_corner_bonus_m)
