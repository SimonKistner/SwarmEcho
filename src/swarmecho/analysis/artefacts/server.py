...
from swarmecho.analysis.artefacts.state import (
    CheckpointOption,
    checkpoint_options,
    cfg_get,
    discover_runs,
...
from swarmecho.analysis.artefacts.jobs import (
    any_running,
    kill_job,
    read_queue,
    start_job,
...
from swarmecho.analysis.artefacts.config import css_vars, load_config
from swarmecho.analysis.artefacts.map_renderer import (
    find_map_yaml,
    load_map_data,
    render_and_save,
    validate_target,
...
def _map_picker_html(run_dir: Path, cfg: dict, tx: float | None, ty: float | None) -> str:
    ...
    if preview is None:
        ...
        return (
            ...
            'Check the bundled swarmecho/curriculum_config/maps directory.</span>'
            '</div>'
        )
...
def _advanced_html(ckpt_opt: CheckpointOption) -> str:
    ...
    return (
        ...
        f'        <div style="font-size:9px; color:var(--text-muted); margin-bottom:6px; line-height:1.2;">Options: --heatmap, --csv, --no-csv</div>'
        f'        <input class="overrides-input" id="pipe-overrides" name="pipe_overrides" type="text" '
        f'          value="--heatmap" style="margin-top:0" placeholder="e.g. --heatmap">'
...
def eval_queue_job(
    ...
    if job == "default_video":
        cmd = ["uv", "run", "swarmecho-evaluate", f"checkpoint={ckpt}"]
        start_job(eval_root, "default_video", cmd, _REPO)
    elif job == "default_heatmaps":
        cmd = ["uv", "run", "swarmecho-evaluate-pipeline", f"checkpoint={ckpt}", "--heatmap"]
        start_job(eval_root, "default_heatmaps", cmd, _REPO)
    elif job == "pipeline":
        extra = (pipe_overrides or "--heatmap").strip().split()
        cmd = ["uv", "run", "swarmecho-evaluate-pipeline", f"checkpoint={ckpt}"] + extra
