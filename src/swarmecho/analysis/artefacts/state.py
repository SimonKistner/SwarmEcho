...
def training_artifacts(run_dir: Path) -> dict[str, list[Path]]:
    ...
    return {
        "videos": _glob_paths([train_new / "vids"], VIDEO_EXTS),
        "found_heatmaps": _glob_paths(
            [train_new / "found_heatmaps"], IMAGE_EXTS
        ),
        "chain_heatmaps": _glob_paths(
            [train_new / "chain_heatmaps"], IMAGE_EXTS
        ),
    }


def eval_artifacts(eval_root: Path) -> dict[str, list[Path]]:
    return {
        "videos": _glob_paths([eval_root / "vids"], VIDEO_EXTS),
