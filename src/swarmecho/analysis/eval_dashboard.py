...
def map_preview_path(run_dir: Path, cfg: dict) -> Path | None:
...
def ensure_map_preview_job(eval_root: Path, run_dir: Path, cfg: dict) -> None:
...
def render_target_picker(eval_root: Path, run_dir: Path, cfg: dict) -> None:
    preview = map_preview_path(run_dir, cfg)
...
with training_tab:
    ...
    video_paths = files_in([train_new / "vids"], VIDEO_EXTS)
    found_paths = files_in([train_new / "found_heatmaps"], IMAGE_EXTS)
    chain_paths = files_in([train_new / "chain_heatmaps"], IMAGE_EXTS)
...
with eval_tab:
    ...
    ensure_map_preview_job(eval_root, run_dir, cfg)
    video_paths = files_in([eval_root / "vids"], VIDEO_EXTS)
    ...
    with video_col:
        ...
        if not video_paths:
            cmd = ["uv", "run", "swarmecho-evaluate", f"checkpoint={selected_ckpt.path}"]
    ...
    with heatmap_col:
        ...
        if not heatmap_paths:
            cmd = ["uv", "run", "swarmecho-evaluate-pipeline", f"checkpoint={selected_ckpt.path}", "--heatmap"]
    ...
    with target_col:
        ...
        render_target_picker(eval_root, run_dir, cfg)
    ...
    with st.expander("Advanced eval tools"):
        pipeline_overrides = st.text_input("Pipeline overrides", value="--heatmap")
        pipeline_cmd = [
            "uv", "run", "swarmecho-evaluate-pipeline",
            f"checkpoint={selected_ckpt.path}",
