...
.artifact-picker{position:relative;width:min(520px,40vw);flex:0 1 520px}.artifact-picker>button{width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;text-align:left;white-space:nowrap}.artifact-picker>button::after{content:'\25be';color:var(--muted)}.artifact-menu{position:absolute;z-index:20;top:calc(100% + 6px);left:0;width:100%;max-height:min(70vh,560px);overflow-y:auto;padding:4px;background:#0c1422;border:1px solid #325173;border-radius:7px;box-shadow:0 14px 32px rgba(0,0,0,.45)}.artifact-item{position:relative}.artifact-item>button{width:100%;display:flex;align-items:center;gap:8px;border:0;border-radius:4px;background:transparent;text-align:left;white-space:nowrap}.artifact-item>button:hover,.artifact-item>button:focus-visible{background:#19304a}.artifact-item.group>button::after{content:'\25b8';margin-left:auto;color:var(--muted)}.artifact-count{min-width:20px;padding:1px 6px;border-radius:10px;background:#294766;color:#d9f7ff;font-size:11px;font-weight:700;text-align:center}.artifact-submenu{position:fixed;z-index:21;min-width:260px;max-width:min(520px,45vw);max-height:min(70vh,560px);overflow:auto;padding:4px;background:#0c1422;border:1px solid #3b5f83;border-radius:7px;box-shadow:0 14px 32px rgba(0,0,0,.55)}.artifact-submenu button{display:block;width:100%;border:0;border-radius:4px;background:transparent;text-align:left;white-space:nowrap}.artifact-submenu button:hover,.artifact-submenu button:focus-visible{background:#19304a}
...
function artifactParts(label){let match=label.match(/^(.*?_\[[^\]]+\])_(\[.*)$/);return match?{prefix:match[1],detail:match[2]}:{prefix:label,detail:label}}
function closeArtifactMenus(){ $('artifactMenu').classList.add('hidden');$('artifactSubmenu').classList.add('hidden');$('artifactPickerButton').setAttribute('aria-expanded','false') }
function chooseArtifact(item,load=true){selectedArtifact=item;$('artifactPickerButton').textContent=item.label;closeArtifactMenus();if(load)loadReplay(item.id)}
function openArtifactSubmenu(group,anchor){let submenu=$('artifactSubmenu');submenu.replaceChildren();group.forEach(item=>{let button=document.createElement('button');button.type='button';button.role='menuitem';button.textContent=artifactParts(item.label).detail;button.onclick=event=>{event.stopPropagation();chooseArtifact(item)};submenu.appendChild(button)});let picker=$('replaySelect').getBoundingClientRect(),row=anchor.getBoundingClientRect();submenu.style.left=Math.min(picker.right+6,window.innerWidth-submenu.offsetWidth-8)+'px';submenu.style.top=Math.min(row.top,window.innerHeight-submenu.offsetHeight-8)+'px';submenu.classList.remove('hidden');let measured=submenu.getBoundingClientRect();submenu.style.left=Math.max(8,Math.min(picker.right+6,window.innerWidth-measured.width-8))+'px';submenu.style.top=Math.max(8,Math.min(row.top,window.innerHeight-measured.height-8))+'px'}
function renderReplayList(items,preferredLabel){let menu=$('artifactMenu'),groups=new Map();menu.replaceChildren();items.forEach(item=>{let prefix=artifactParts(item.label).prefix;if(!groups.has(prefix))groups.set(prefix,[]);groups.get(prefix).push(item)});groups.forEach((group,prefix)=>{let row=document.createElement('div'),button=document.createElement('button');row.className='artifact-item'+(group.length>1?' group':'');button.type='button';button.role='menuitem';if(group.length===1){button.textContent=group[0].label;button.onclick=event=>{event.stopPropagation();chooseArtifact(group[0])}}else{let name=document.createElement('span'),count=document.createElement('span');name.textContent=prefix;count.className='artifact-count';count.textContent=group.length;button.append(name,count);button.onclick=event=>{event.stopPropagation();openArtifactSubmenu(group,button)}}row.appendChild(button);menu.appendChild(row)});let choice=items.find(x=>x.label===preferredLabel)||items.find(x=>x.selected)||items[0];if(choice)chooseArtifact(choice,false);return Promise.resolve()}
function refreshReplays(){let preferredLabel=selectedArtifact?.label,b=$('refreshReplays');b.disabled=true;setLoading(true,'Finding artifacts...');$('replayDiscovery').textContent='Finding artifacts...';fetch('/api/replays').then(r=>{if(!r.ok)throw Error('Artifact list failed to load');return r.json()}).then(items=>renderReplayList(items,preferredLabel).then(()=>{$('replayDiscovery').textContent=items.length+' artifact'+(items.length===1?'':'s')+' found'})).catch(()=>{$('replayDiscovery').textContent='Unable to find artifacts'}).finally(()=>{b.disabled=false;setLoading(false)})}
function loadSelectedReplay(){if(selectedArtifact)loadReplay(selectedArtifact.id)}
function refreshCurrent(){let id=selectedArtifact?.id,b=$('refreshCurrent');if(!id)return;b.disabled=true;loadReplay(id,true).finally(()=>{b.disabled=false})}
$('artifactPickerButton').onclick=event=>{event.stopPropagation();let menu=$('artifactMenu'),opening=menu.classList.contains('hidden');closeArtifactMenus();menu.classList.toggle('hidden',!opening);$('artifactPickerButton').setAttribute('aria-expanded',String(opening))};document.addEventListener('click',closeArtifactMenus);window.addEventListener('resize',closeArtifactMenus);$('loadReplay').onclick=loadSelectedReplay;$('refreshCurrent').onclick=refreshCurrent;$('refreshReplays').onclick=refreshReplays;refreshReplays();
