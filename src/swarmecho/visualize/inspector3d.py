...
<div class="card"><div class="label">Playback</div><div class="controls"><button id="play">▶ Play</button><button id="step">Step</button></div><input id="timeline" type="range" min="0" value="0"><div class="row"><span>Frame</span><b id="frame">0</b></div><div class="row"><span>Time</span><b id="time">0.0 s</b></div><select id="speed"><option value="0.25">0.25×</option><option value="0.5">0.5×</option><option selected value="1">1×</option><option value="2">2×</option><option value="4">4×</option></select><label style="display:block;margin-top:10px"><input id="loopReplay" type="checkbox"> Loop episode</label><div class="label" style="margin-top:16px">Camera</div><label class="layer-control"><span><input id="autoRotate" type="checkbox"> Auto rotate</span><span id="rotateSpeedValue" class="opacity-value">20°/s</span></label><input id="rotateSpeed" type="range" min="-90" max="90" step="1" value="20" aria-label="Camera rotation speed in degrees per second"></div>
...
let D,H,mode='replay',frame=0,playing=false,timer=null,camera=null,heatmapAgreement=1,rotationFrame=null,rotationTime=null;const $=id=>document.getElementById(id);const DEFAULT_CAMERA={eye:{x:1.45,y:1.45,z:.75},projection:{type:'perspective'}};
function worldSize(){return D?.manifest?.world_size_m||H?.manifest?.world_size_m||[20,20,20]}
function sceneAxes(){let size=worldSize(),rangeRadius=mode==='replay'?Math.max($('showVisualRange').checked?(D?.manifest?.visual_radius_m||4):0,$('showCommRange').checked?Math.max(D?.manifest?.comm_radius_m||5,D?.manifest?.comm_radius_base_m||6):0):0,axis=(title,maximum)=>({title,gridcolor:'#22304a',range:[-rangeRadius,maximum+rangeRadius],autorange:false});return {xaxis:axis('X',size[0]),yaxis:axis('Y',size[1]),zaxis:axis('Z',size[2])}}
...
const renderPromise=Plotly.react('scene',tr,{margin:{l:0,r:0,t:0,b:0},paper_bgcolor:'#090e18',uirevision:'replay-camera',scene:sceneLayout('replay-camera'),showlegend:false},{responsive:true,displaylogo:false}).then(captureCamera);
...
$('timeline').oninput=e=>draw(e.target.value);['showCoverage','showVisualRange','showCommRange','showLinks','showShell'].forEach(id=>$(id).onchange=()=>draw(frame));[['coverageOpacity','coverageOpacityValue'],['visualOpacity','visualOpacityValue'],['commOpacity','commOpacityValue']].forEach(([input,output])=>{updateOpacityValue(input,output);$(input).oninput=()=>{updateOpacityValue(input,output);draw(frame)}});Object.values(HEATMAP_CATEGORY_CONTROLS).forEach(id=>$(id).onchange=()=>{if(mode==='heatmap'){rememberCamera();drawHeatmap()}});$('heatmapConfidence').oninput=()=>{$('heatmapConfidenceValue').textContent=$('heatmapConfidence').value+'%';if(mode==='heatmap'){rememberCamera();drawHeatmap()}};$('copyHeatmapCommand').onclick=()=>{let command=$('heatmapCommand').value;if(command)navigator.clipboard.writeText(command).then(()=>{$('copyHeatmapCommand').textContent='Copied';setTimeout(()=>{$('copyHeatmapCommand').textContent='Copy command'},1200)})};$('autoRotate').onchange=updateAutoRotate;$('rotateSpeed').oninput=()=>{$('rotateSpeedValue').textContent=$('rotateSpeed').value+'°/s'};$('step').onclick=()=>draw(Math.min(frame+1,D.manifest.frames-1));$('play').onclick=()=>{playing=!playing;$('play').textContent=playing?'❚❚ Pause':'▶ Play';if(playing)tick();else clearTimeout(timer)};function tick(){if(!playing)return;let last=D.manifest.frames-1;if(frame>=last&&!$('loopReplay').checked){playing=false;$('play').textContent='▶ Play';timer=null;return}draw(frame>=last?0:frame+1);timer=setTimeout(tick,1000*D.manifest.dt/+$('speed').value)}
</script></body></html>"""
