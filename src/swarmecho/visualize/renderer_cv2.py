...
from swarmecho.visualize.renderer_config import RendererConfig
...
class _FrameData(NamedTuple):
    ...
    directly_sees_target: np.ndarray | None
...
def _draw_frame_cv2(
    ...
    # Translate the physics-owned graph from [drones..., base] into the
    # renderer's [base, target, drones...] node order. Target edges come from
    # the compact direct-visibility vector computed by the same physics pass.
    adj = np.zeros((M, M), dtype=bool)
    dists = np.linalg.norm(ents[:, None] - ents[None, :], axis=-1)
    if frame.adj_matrix is not None and frame.adj_matrix.shape == (N + 1, N + 1):
        physics_adj = np.asarray(frame.adj_matrix, dtype=bool)
        adj[drone_start:, drone_start:] = physics_adj[:N, :N]
        adj[base_idx, drone_start:] = physics_adj[N, :N]
        adj[drone_start:, base_idx] = physics_adj[:N, N]
    if (
        frame.directly_sees_target is not None
        and len(frame.directly_sees_target) == N
    ):
        target_edges = np.asarray(frame.directly_sees_target, dtype=bool)
        adj[target_idx, drone_start:] = target_edges
        adj[drone_start:, target_idx] = target_edges
