...
uv run swarmecho-render M03_big_maze --mode image

# 3. Render a clean architectural SVG blueprint without spawns/zones:
uv run swarmecho-render M03_big_maze --mode image --format svg --no-spawns --no-zones
...
from swarmecho.core.config import MAP_DIR, load_config, validate_config
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional exact output path; standalone previews use outputs/map_previews by default",
    )
