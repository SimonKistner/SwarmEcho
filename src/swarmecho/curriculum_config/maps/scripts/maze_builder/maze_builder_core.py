...
from swarmecho.core.config import LEVEL_DIR, MAP_DIR
...
def create_map_and_level(payload: dict[str, Any], overwrite: bool = False) -> dict[str, Any]:
    ...
    preview_command = f"uv run swarmecho-render {map_name} --mode image --format png --no-spawns"
    return {
