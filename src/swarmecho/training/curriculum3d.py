...
from swarmecho.training.train3d import train_3d
...
def main() -> None:
    ...
    base_level = load_level_3d_cli(shared_args)
    ...
    for index, level_name in enumerate(levels, start=1):
        level = load_level_3d_cli([f"level={level_name}", *shared_args])
        ...
        final_checkpoint, _ = train_3d(level)
