...
# ---------------------------------------------------------------------------
# Map preview job helper
# ---------------------------------------------------------------------------

def ensure_map_preview_job(
    run_dir: Path,
    map_preview_png: Path,
    map_name: str,
    repo_root: Path,
) -> bool:
    """
    If the map preview PNG does not exist, queue a render job (once).
    Stores the job file in the run-level artifacts/jobs/ dir so it is shared
    across all checkpoints.
    Returns True if a new job was launched.
    """
    if map_preview_png.exists():
        return False
    ...
    command = [
        "uv", "run", "python",
        "src/visualize/render_preview.py",
        map_name,
        "--mode", "image",
        "--format", "png",
        "--no-spawns",
