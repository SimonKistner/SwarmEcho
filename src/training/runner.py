    ...
    *,
    ...
    if recurrent:
        if memory_comm_enabled:
            ...
            removed_comm_gate_enabled = False
            ...
            def _rollout_one_env(obs_n, keys_n, actor_h_n, actor_sig_n, actor_val_n, critic_h_n, resets_n, comm_mask_n, active_n, base_sig_n, base_val_n, base_memory_mask_n):
                gate_actions_n = jnp.ones(actor_h_n.shape[:-1], dtype=bool)
                gate_log_probs_n = jnp.zeros(actor_h_n.shape[:-1], dtype=jnp.float32)
                gate_entropy_n = jnp.zeros(actor_h_n.shape[:-1], dtype=jnp.float32)
                gate_prob_n = jnp.ones(actor_h_n.shape[:-1], dtype=jnp.float32)
                ...
                return actor_h_out, actor_sig_out, actor_val_out, critic_h_out, actions_n, log_probs_n, values_n, gate_actions_n, gate_log_probs_n, gate_entropy_n, gate_prob_n

            actor_h, actor_signature, actor_value, critic_h, actions_b, log_probs_b, values_b, comm_gate_actions_b, comm_gate_log_probs_b, comm_gate_entropy_b, comm_gate_prob_b = jax.vmap(_rollout_one_env)(
            ...
            gate_decision_active_b = jnp.full(comm_gate_actions_b.shape, removed_comm_gate_enabled & step_share, dtype=bool) & active_mask_b
...
def _collect_rollout_mappo(
    ...
    completed_r_succ     = []
    completed_coverage   = []
    completed_removed_comm_gate_rates = []
    completed_removed_comm_gate_target_known_rates = []
    completed_removed_comm_gate_gate_prob_means = []
    completed_removed_comm_gate_gate_entropy_means = []

    removed_comm_gate_comm_count_accum = ep_trackers.get("removed_comm_gate_comm_count", np.zeros(E))
    removed_comm_gate_comm_opportunity_accum = ep_trackers.get("removed_comm_gate_comm_opportunity", np.zeros(E))
    removed_comm_gate_target_known_comm_count_accum = ep_trackers.get("removed_comm_gate_target_known_comm_count", np.zeros(E))
    removed_comm_gate_target_known_opportunity_accum = ep_trackers.get("removed_comm_gate_target_known_opportunity", np.zeros(E))
    removed_comm_gate_gate_prob_sum_accum = ep_trackers.get("removed_comm_gate_gate_prob_sum", np.zeros(E))
    removed_comm_gate_gate_entropy_sum_accum = ep_trackers.get("removed_comm_gate_gate_entropy_sum", np.zeros(E))
    removed_comm_gate_step_count_accum = ep_trackers.get("removed_comm_gate_step_count", np.zeros(E))
    ...
    for t in range(T):
        ...
        if model.actor_memory and model.memory_comm_enabled and False:
            ...
            removed_comm_gate_comm_count_accum += gate_sum_np
            removed_comm_gate_comm_opportunity_accum += opportunity_np
            removed_comm_gate_target_known_comm_count_accum += gate_sum_np * target_known_step_np
            removed_comm_gate_target_known_opportunity_accum += opportunity_np * target_known_step_np
            prob_denom_np = np.maximum(opportunity_np, 1.0)
            removed_comm_gate_gate_prob_sum_accum += gate_prob_np.sum(axis=1) / prob_denom_np
            removed_comm_gate_gate_entropy_sum_accum += gate_entropy_np.sum(axis=1) / prob_denom_np
            removed_comm_gate_step_count_accum += (opportunity_np > 0.0).astype(np.float32)
        ...
        for e in done_envs:
            ...
            if model.actor_memory and model.memory_comm_enabled and False:
                denom = max(1.0, float(removed_comm_gate_comm_opportunity_accum[e]))
                completed_removed_comm_gate_rates.append(float(removed_comm_gate_comm_count_accum[e]) / denom)
                known_denom = max(1.0, float(removed_comm_gate_target_known_opportunity_accum[e]))
                completed_removed_comm_gate_target_known_rates.append(float(removed_comm_gate_target_known_comm_count_accum[e]) / known_denom)
                step_denom = max(1.0, float(removed_comm_gate_step_count_accum[e]))
                completed_removed_comm_gate_gate_prob_means.append(float(removed_comm_gate_gate_prob_sum_accum[e]) / step_denom)
                completed_removed_comm_gate_gate_entropy_means.append(float(removed_comm_gate_gate_entropy_sum_accum[e]) / step_denom)
        ...
        removed_comm_gate_comm_count_accum = np.where(dones_np, 0.0, removed_comm_gate_comm_count_accum)
        removed_comm_gate_comm_opportunity_accum = np.where(dones_np, 0.0, removed_comm_gate_comm_opportunity_accum)
        removed_comm_gate_target_known_comm_count_accum = np.where(dones_np, 0.0, removed_comm_gate_target_known_comm_count_accum)
        removed_comm_gate_target_known_opportunity_accum = np.where(dones_np, 0.0, removed_comm_gate_target_known_opportunity_accum)
        removed_comm_gate_gate_prob_sum_accum = np.where(dones_np, 0.0, removed_comm_gate_gate_prob_sum_accum)
        removed_comm_gate_gate_entropy_sum_accum = np.where(dones_np, 0.0, removed_comm_gate_gate_entropy_sum_accum)
        removed_comm_gate_step_count_accum = np.where(dones_np, 0.0, removed_comm_gate_step_count_accum)
    ...
    ep_trackers["removed_comm_gate_comm_count"] = removed_comm_gate_comm_count_accum
    ep_trackers["removed_comm_gate_comm_opportunity"] = removed_comm_gate_comm_opportunity_accum
    ep_trackers["removed_comm_gate_target_known_comm_count"] = removed_comm_gate_target_known_comm_count_accum
    ep_trackers["removed_comm_gate_target_known_opportunity"] = removed_comm_gate_target_known_opportunity_accum
    ep_trackers["removed_comm_gate_gate_prob_sum"] = removed_comm_gate_gate_prob_sum_accum
    ep_trackers["removed_comm_gate_gate_entropy_sum"] = removed_comm_gate_gate_entropy_sum_accum
    ep_trackers["removed_comm_gate_step_count"] = removed_comm_gate_step_count_accum
...
def _run_parallel_eval_jit(
    ...
    def run_single_env_rollout(env_key):
        ...
        init_carry = (
            ...
            jnp.int32(0),      # lengths
            jnp.float32(0.0),  # removed_comm_gate_gate_count
            jnp.float32(0.0),  # removed_comm_gate_gate_opportunity
        )

        def eval_step(carry, t):
            state, actor_h, actor_signature, actor_value, base_signature, base_value, base_memory_valid, has_succeeded, has_found_delivered, has_found_visual, max_found, returns, lengths, removed_comm_gate_gate_count, removed_comm_gate_gate_opportunity = carry
            ...
            new_removed_comm_gate_gate_count = jnp.where(episode_ended_prev, removed_comm_gate_gate_count, removed_comm_gate_gate_count + gate_count_step)
            new_removed_comm_gate_gate_opportunity = jnp.where(episode_ended_prev, removed_comm_gate_gate_opportunity, removed_comm_gate_gate_opportunity + gate_opportunity_step)

            new_carry = (state, actor_h, actor_signature, actor_value, base_signature, base_value, base_memory_valid, new_has_succeeded, new_has_found_delivered, new_has_found_visual, new_max_found, new_returns, new_lengths, new_removed_comm_gate_gate_count, new_removed_comm_gate_gate_opportunity)
        ...
        state_f, _, _, _, _, _, _, succ, delivered, visual, fnd, ret, length, removed_comm_gate_gate_count, removed_comm_gate_gate_opportunity = final_carry
        gap_dists, progress_pcts = scan_outs

        denom = jnp.maximum(jnp.float32(1.0), removed_comm_gate_gate_opportunity)
        removed_comm_gate_rate = removed_comm_gate_gate_count / denom
        return ret, length, gap_dists[-1], progress_pcts[-1], succ, fnd, state_f, succ, delivered, visual, removed_comm_gate_rate
...
def _evaluate_parallel(
    ...
    Returns:
      rets, lengths, gaps, progs, succs, fnds, final_state, final_succs, final_delivered, final_visual, removed_comm_gate_rates
    """
...
def train(cfg: DictConfig, success_threshold: Optional[float] = None):
    ...
    window_removed_comm_gate_rates = deque(maxlen=E)
    window_removed_comm_gate_target_known_rates = deque(maxlen=E)
    window_removed_comm_gate_gate_prob_means = deque(maxlen=E)
    window_removed_comm_gate_gate_entropy_means = deque(maxlen=E)
    window_removed_comm_gate_success_rates = deque(maxlen=E)
    window_removed_comm_gate_failure_rates = deque(maxlen=E)
    ...
    try:
        for update in range(start_update + 1, n_updates + 1):
            ...
            (states, collect_key,
             ...
             raw_cov,
             raw_finder_return, raw_target_positions, raw_diag_memories, raw_diag_targets, raw_diag_valids,
             raw_removed_comm_gate_rates, raw_removed_comm_gate_target_known_rates, raw_removed_comm_gate_gate_prob_means, raw_removed_comm_gate_gate_entropy_means) = _collect_rollout_mappo(
            ...
            if n_eps > 0:
                ...
                if False:
                    window_removed_comm_gate_rates.extend(raw_removed_comm_gate_rates)
                    window_removed_comm_gate_target_known_rates.extend(raw_removed_comm_gate_target_known_rates)
                    window_removed_comm_gate_gate_prob_means.extend(raw_removed_comm_gate_gate_prob_means)
                    window_removed_comm_gate_gate_entropy_means.extend(raw_removed_comm_gate_gate_entropy_means)
                    for rate, succ in zip(raw_removed_comm_gate_rates, raw_success):
                        (window_removed_comm_gate_success_rates if succ >= 0.5 else window_removed_comm_gate_failure_rates).append(rate)
            ...
            if wandb_run:
                ...
                if len(window_ret) == window_ret.maxlen:
                    ...
                    if False:
                        removed_comm_gate_threshold = float(cfg.network.get("removed_comm_gate_comm_always_threshold", 0.95))
                        logs.update({
                            "comm/removed_comm_gate_rate_mean": float(np.mean(window_removed_comm_gate_rates)),
                            "comm/removed_comm_gate_rate_std": float(np.std(window_removed_comm_gate_rates)),
                            "comm/removed_comm_gate_always_true_episode_pct": float(np.mean(np.asarray(window_removed_comm_gate_rates) >= removed_comm_gate_threshold)),
                            "comm/removed_comm_gate_always_false_episode_pct": float(np.mean(np.asarray(window_removed_comm_gate_rates) <= (1.0 - removed_comm_gate_threshold))),
                            "comm/removed_comm_gate_rate_when_target_known": float(np.mean(window_removed_comm_gate_target_known_rates)) if len(window_removed_comm_gate_target_known_rates) > 0 else 0.0,
                            "comm/removed_comm_gate_gate_prob_mean": float(np.mean(window_removed_comm_gate_gate_prob_means)) if len(window_removed_comm_gate_gate_prob_means) > 0 else 0.0,
                            "comm/removed_comm_gate_gate_entropy_mean": float(np.mean(window_removed_comm_gate_gate_entropy_means)) if len(window_removed_comm_gate_gate_entropy_means) > 0 else 0.0,
                        })
                        if len(window_removed_comm_gate_success_rates) > 0:
                            logs["comm/removed_comm_gate_rate_success"] = float(np.mean(window_removed_comm_gate_success_rates))
                        if len(window_removed_comm_gate_failure_rates) > 0:
                            logs["comm/removed_comm_gate_rate_failure"] = float(np.mean(window_removed_comm_gate_failure_rates))
                ...
                if bool(cfg.training.get("eval_parallel", False)):
                    if is_eval_step:
                        ...
                        (rets, lengths, gaps, progs, succs, fnds,
                         final_state, final_succs, final_delivered, final_visual, eval_removed_comm_gate_rates) = _evaluate_parallel(
                        ...
                        eval_removed_comm_gate_rate = float(jnp.mean(eval_removed_comm_gate_rates)) if False else 0.0
                        ...
                        if wandb_run:
                            ...
                            wandb.log({
                                ...
                                **({"eval/comm/removed_comm_gate_rate_mean": eval_removed_comm_gate_rate} if False else {}),
                            }, step=steps_done)
