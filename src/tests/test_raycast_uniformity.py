...
def run_test(map_name=None):
    ...
    # Define the explicit edge cases from the reported mismatches (only run if using the default grid to avoid out-of-bounds)
    if map_name is None:
        edge_cases = [
    ...
    print(f"\nRunning a batch of 500 random paths on grid of size {W}x{H}...")
    mismatches = 0
    np.random.seed(42)
    ...
    for _ in tqdm(range(500), desc="Verifying raycasts"):
        ...
        sp = np.random.uniform(1.0, W - 1.0, 2)
        ep = np.random.uniform(1.0, H - 1.0, 2)
    ...
    if mismatches > 0:
        ...
        print(f"\n✅ SUCCESS: Raycast results are 100% identical on all 500 random paths for map '{map_name or 'default'}'!")

if __name__ == "__main__":
    map_arg = None
    for arg in sys.argv[1:]:
        if arg.startswith("map="):
            map_arg = arg.split("=", 1)[1]
